...
class RemoteScannersActor(injector: Injector) extends FSM[State, Data] with LazyLogging {
  ...
  val path = s"${settings.remoteServer.path}/user/root/scanners-endpoint-actor"
  ...
  def convertEvent(terminalId: Id[Terminal], event: Message) = {
    event match {
